...
//	});
	...
	module.AttendanceSlider.include({ 
			start: function() {
	        	...
	        	self.$(".oe_attendance_signout").click(function() {
...
//							self.obj.dispatch_to_new_action();
					    	...
					    	mod1.call('desc_pop_start',[result.list_id,result]).done(function(ctx){
								...
								instance.client.action_manager.do_action(action).done(function(){
									...
									$('.ui-dialog-titlebar-close.ui-corner-all').on('click',function(e){
...
//											});
							...
							mod1.call('desc_pop_start',[result.list_id,result]).done(function(ctx){
								...
								$("body").on('click',function(e){
									if ($(e.srcElement).hasClass("ui-dialog-titlebar-close ui-corner-all")){
	...
	var modules = instance.mail_followers;// loading the namespace of the 'sample' module
	modules.Followers.include({
		start: function (){
